...
>>>>>>> parent of 160b873... Actual ES6 conventions (kinda)
...
<<<<<<< HEAD
  let from = rates.find(f => { return f.token === req.headers.authorization });
  if (from === undefined) {
  ...
  let rate = rates.find(r => { return r.code === req.params.to });
  if (rate === undefined) {
    res.sendRaw('[ERROR] "To" currency NOT FOUND.');
    return;
  }
  let amount = parseInt(req.params.amount) * from.from * rate.to;
...
server.get('/transaction', function respond(req, res, next) {
  let bot = rates.find(f => { return f.token === req.headers.authorization });
  if (bot === undefined) {
    res.sendRaw('[ERROR] Unauthorized token!');
    return;
  }
